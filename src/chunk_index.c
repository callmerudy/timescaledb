...
static bool
chunk_index_chunk_schema_filter(TupleInfo *ti, void *data)
{
	FormData_chunk_index *chunk_index = (FormData_chunk_index *) GETSTRUCT(ti->tuple);
	ChunkIndexDeleteData *cid = data;
	Chunk	   *chunk = chunk_get_by_id(chunk_index->chunk_id, 0, false);

	return NULL != chunk && namestrcmp(&chunk->fd.schema_name, cid->schema) == 0;
}

static bool
chunk_index_hypertable_schema_filter(TupleInfo *ti, void *data)
{
	FormData_chunk_index *chunk_index = (FormData_chunk_index *) GETSTRUCT(ti->tuple);
	ChunkIndexDeleteData *cid = data;
	Hypertable *ht = hypertable_get_by_id(chunk_index->hypertable_id);

	return NULL != ht && namestrcmp(&ht->fd.schema_name, cid->schema) == 0;
}
...
chunk_index_delete_by_name(const char *schema, const char *index_name, bool drop_index)
{
	...
	ScanKeyInit(&scankey[0],
				Anum_chunk_index_chunk_id_index_name_idx_index_name,
				BTEqualStrategyNumber, F_NAMEEQ, DirectFunctionCall1(namein, CStringGetDatum(index_name)));

	/* scan for any chunk indexes by that name */
	chunk_index_scan_update(CHUNK_INDEX_CHUNK_ID_INDEX_NAME_IDX,
							scankey, 1, chunk_index_tuple_delete, chunk_index_chunk_schema_filter, &data);

	ScanKeyInit(&scankey[0],
				Anum_chunk_index_hypertable_id_hypertable_index_name_idx_hypertable_index_name,
				BTEqualStrategyNumber, F_NAMEEQ, DirectFunctionCall1(namein, CStringGetDatum(index_name)));

	/* scan for any hypertable indexes by that name */
	chunk_index_scan_update(CHUNK_INDEX_HYPERTABLE_ID_HYPERTABLE_INDEX_NAME_IDX,
							scankey, 1, chunk_index_tuple_delete, chunk_index_hypertable_schema_filter, &data);
}
